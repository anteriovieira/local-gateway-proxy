...
export const Home: React.FC<HomeProps> = ({ workspaces, activeWorkspaceId, onSelectWorkspace, onAddWorkspace, onReorderWorkspaces, onRemoveWorkspace, onToggleServer, onDuplicateWorkspace, variant = 'desktop' }) => {
    ...
    <div className="flex-1 flex flex-col bg-zinc-950 overflow-hidden">
      <div className="flex flex-row justify-between items-center p-3 border-b border-zinc-900 bg-zinc-900/30 text-xs font-medium text-zinc-400 shrink-0">
        <span className="flex items-center gap-2 flex-shrink-0">
          ...
          {workspaces.length > 0 && (
            <>
              ...
              <span className="text-zinc-600 normal-case">{workspaces.length} {workspaces.length === 1 ? 'workspace' : 'workspaces'}</span>
              <span className="text-zinc-600">•</span>
              <span className={cn("flex items-center gap-1.5", workspaces.some(w => w.isRunning) ? "text-emerald-500" : "text-zinc-600")}>
                <Activity className={cn("w-3.5 h-3.5", workspaces.some(w => w.isRunning) && "animate-pulse")} />
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 rounded-md text-zinc-300 hover:text-white text-xs font-medium transition-colors"
        >
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 text-xs text-zinc-500">
                      {variant === 'desktop' && <span className="font-mono">http://localhost:{ws.port}</span>}
                      ...
                      {openDropdownId === ws.id && (
                        <div className="absolute right-0 top-full mt-1 w-48 bg-zinc-900 border border-zinc-800 rounded-lg shadow-xl z-50 overflow-hidden">
                          <div className="py-1">
                            ...
                            >
                              {ws.isRunning ? (
                                <><Square className="w-4 h-4 fill-current" /><span>Stop Server</span></>
                              ) : (
                                <><Play className="w-4 h-4 fill-current" /><span>Start Server</span></>
                              )}
                            </button>
      ...
      {workspaceToDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={cancelDelete}>
          <div className="bg-zinc-900 border border-zinc-800 rounded-lg p-6 max-w-[90vw] max-h-[90vh] w-full mx-4 shadow-2xl overflow-auto" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-white mb-2">Delete Workspace</h3>
            <p className="text-sm text-zinc-400 mb-6">
              Are you sure you want to delete <span className="font-medium text-white">"{workspaceToDelete.name}"</span>? This action cannot be undone.
            </p>
            <div className="flex items-center justify-end gap-3">
              <button onClick={cancelDelete} className="px-4 py-2 rounded-md bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-sm transition-colors">Cancel</button>
              <button onClick={confirmDelete} className="px-4 py-2 rounded-md bg-red-500 hover:bg-red-600 text-white text-sm transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
