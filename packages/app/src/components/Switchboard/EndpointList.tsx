    ...
    <div className="h-full flex flex-col overflow-hidden">
          ...
          {METHODS.map((method) => (
            <button
              ...
              className={cn(
                ...
                methodFilter === method ? (method === 'ALL' ? "bg-zinc-700 text-white" : getMethodBgColor(method)) : "bg-zinc-900 text-zinc-500 hover:bg-zinc-800"
              )}
            >
              {method}
            </button>
      ...
      <div ref={parentRef} className="flex-1 overflow-auto pr-2 custom-scrollbar">
        {filteredEndpoints.length === 0 ? (
          <div className="text-center text-zinc-600 text-xs py-8">No endpoints match your filter</div>
        ) : (
                ...
                >
                  <div
                    className={cn(
                      ...
                      ep.enabled !== false ? "bg-zinc-900/50 border-zinc-800 text-zinc-200" : "bg-zinc-950 border-zinc-900 text-zinc-600 opacity-60"
                    )}
                  >
                    <button
                      onClick={(e) => { e.stopPropagation(); onToggle(originalIndex) }}
                      className={cn(
                    ...
                    <span className={cn("font-mono font-bold px-2 py-0.5 rounded text-[10px] w-14 text-center select-none shrink-0", getMethodColor(ep.method))}>
                      {ep.method.toUpperCase()}
                    </span>
                    <button
                      onClick={() => onEndpointClick?.(ep.path)}
                      className={cn("font-mono truncate flex-1 text-left hover:text-purple-400 transition-colors", onEndpointClick && "cursor-pointer")}
